...
EOF`,
  ...
  },
  {
    ...
    heading: 'kirkware-all-models, with claude-sonnet-4-6 as the default',
    description: 'The chat UI lets a user pick among every model the bound plan exposes — `ANTHROPIC_MODEL` just sets which one answers by default when nobody picks.',
    commands: [
      {
      ...
      {
        label: 'set-default-model.sh',
        lang: 'bash',
        code: `cf set-env kirkwaregpt ANTHROPIC_MODEL claude-sonnet-4-6`,
        liveId: 'kirkwaregpt-set-default-model.sh',
      },
      {
        label: 'restage.sh',
        lang: 'bash',
  ...
  },
  {
    ...
    commands: [
      ...
      {
        label: 'create-postgres.sh',
        lang: 'bash',
        code: `cf create-service postgres "$POSTGRES_PLAN" kirkwaregpt-db --wait`,
        liveId: 'kirkwaregpt-create-postgres.sh',
      },
      {
    ...
    impact: 'Set `POSTGRES_PLAN` in Settings to whatever your foundation\'s Postgres offering actually calls its plan — confirmed back in Lab Preparation\'s marketplace check.',
  },
  {
    id: 'agent-diagram-3',
    type: 'diagram',
