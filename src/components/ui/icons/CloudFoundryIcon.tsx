import { useId } from 'react'
import type { LucideProps } from 'lucide-react'

/** The Cloud Foundry mark — fixed brand colors (gray cloud + gradient droplet), unlike the currentColor-based Lucide icons. */
export function CloudFoundryIcon({ size = 24, className }: LucideProps) {
  // The gradient id must be unique per rendered instance, or multiple icons on
  // the same page would collide on the same `id="a"` and all reference
  // whichever instance's <linearGradient> happens to win in the DOM.
  const gradientId = useId()

  return (
    <svg viewBox="0 0 32 32" width={size} height={size} className={className}>
      <defs>
        <linearGradient
          id={gradientId}
          x1="0.879"
          y1="34.702"
          x2="1.093"
          y2="34.702"
          gradientTransform="translate(4527.333 531.419) rotate(98.13) scale(130.809)"
          gradientUnits="userSpaceOnUse"
        >
          <stop offset="0" stopColor="#60baeb" />
          <stop offset="0.041" stopColor="#60baeb" />
          <stop offset="0.129" stopColor="#4fb4e9" />
          <stop offset="0.822" stopColor="#4054ad" />
          <stop offset="1" stopColor="#4054ad" />
        </linearGradient>
      </defs>
      <path
        fill="#737373"
        d="M13.134,18.662s.488-1.85-1.633-4.234l-.071-.075a5.873,5.873,0,0,1,4.5-9.662h.137a5.873,5.873,0,0,1,4.5,9.661c-.011.012-.035.037-.071.073a5.622,5.622,0,0,0-1.634,4.237A8.567,8.567,0,0,0,16.489,2h-.978a8.567,8.567,0,0,0-2.377,16.659"
      />
      <path
        fill="#737373"
        d="M24.242,25.484a2.65,2.65,0,0,1,.514-.15,3.264,3.264,0,0,1,.6-.072s-.01-.067-.023-.167a1.566,1.566,0,0,0-.1-.358l-.066-.186c-.026-.059-.06-.113-.084-.159L25,24.239a5.266,5.266,0,0,1-.614-.139,4.785,4.785,0,0,1-.554-.191,2.39,2.39,0,0,1-.765-.465.561.561,0,0,1-.217-.477.409.409,0,0,1,.1-.21.655.655,0,0,1,.223-.174l-.186-.1c-.056-.03-.118-.066-.189-.1l-.234-.112c-.156-.074-.312-.149-.443-.2l-.207-.084a4.115,4.115,0,0,1-.482.034c-.173,0-.349,0-.52-.014a5.06,5.06,0,0,1-.977-.163,2.421,2.421,0,0,1-.4-.138,1.826,1.826,0,0,1-.3-.157c-.156-.114-.263-.229-.262-.345l-.236-.04c-.146-.023-.458-.065-.656-.091,0,.033-.025.065-.032.12l-.05.242a.951.951,0,0,0,.654,1.114,3.33,3.33,0,0,1,2.3,1.585c.514,1.388-1.647,2.744-4.934,2.744s-5.449-1.356-4.936-2.744A3.263,3.263,0,0,1,13.3,22.575a1.007,1.007,0,0,0,.686-1.172l-.069-.33-.439.054c-.2.022-.381.056-.524.079l-.239.04a.429.429,0,0,1-.224.349,1.941,1.941,0,0,1-.278.164,3.363,3.363,0,0,1-.384.144,3.782,3.782,0,0,1-.469.108,4.615,4.615,0,0,1-.5.068,5.123,5.123,0,0,1-1.017,0l-.058.023c-.035.015-.091.036-.147.064l-.432.206c-.156.078-.3.162-.408.224l-.136.076-.047.031a.57.57,0,0,1,.366.386c.055.155-.021.315-.163.491a2.125,2.125,0,0,1-.308.254,2.775,2.775,0,0,1-.442.235A4.789,4.789,0,0,1,6.9,24.43a.732.732,0,0,0-.067.156c-.034.095-.078.223-.124.354-.019.13-.037.263-.052.365a.578.578,0,0,0-.01.17,3.291,3.291,0,0,1,1.161.2,1.433,1.433,0,0,1,.728.5.8.8,0,0,1,.066.746,1.774,1.774,0,0,1-.661.844L8,27.807l.167.119c.143.1.323.237.534.363l.621.341a1.79,1.79,0,0,0,.226.1l.089.038a5.531,5.531,0,0,1,1.6-.386,4.694,4.694,0,0,1,.8-.019,4.336,4.336,0,0,1,.774.092,4.661,4.661,0,0,1,.7.22,4.007,4.007,0,0,1,.583.33,1.927,1.927,0,0,1,.753.954l.462.018c.278.007.649.032,1.018.013s.74-.018,1.015-.036l.452-.043a1.4,1.4,0,0,1,.237-.53,1.716,1.716,0,0,1,.193-.225,2.337,2.337,0,0,1,.224-.213A2.361,2.361,0,0,1,19,28.606a3.2,3.2,0,0,1,.667-.24,5.926,5.926,0,0,1,.752-.119,6.751,6.751,0,0,1,.792,0,5.613,5.613,0,0,1,1.618.342l.281-.156a4.964,4.964,0,0,0,.57-.351c.181-.134.372-.262.487-.367l.186-.175a2.148,2.148,0,0,1-.734-.816.9.9,0,0,1-.087-.381A.547.547,0,0,1,23.606,26a1.207,1.207,0,0,1,.637-.513"
      />
      <path
        fill={`url(#${gradientId})`}
        d="M11.641,10.589A3.5,3.5,0,0,0,13,13.334h0a6.165,6.165,0,0,1,1.954,4.456h0c.055,2.088-.088,4.5-.175,5.735h0c-1.118.158-1.9.532-1.9.968h0c0,.579,1.391,1.249,3.108,1.249h0c1.717,0,3.108-.669,3.108-1.249h0c0-.436-.785-.81-1.9-.968h0c-.088-1.23-.229-3.647-.174-5.735h0a6.165,6.165,0,0,1,1.954-4.456h0a3.5,3.5,0,0,0,1.359-2.745h0c-.018-2.058-1.962-3.715-4.342-3.715h0c-2.381,0-4.325,1.656-4.342,3.715"
      />
    </svg>
  )
}
